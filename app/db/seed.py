    ...
    async with async_session_maker() as session:
        ...
        for cat in CATEGORIES:
            existing = await session.scalar(
                select(PositionCategory).where(PositionCategory.code == cat["code"])
            )
            if existing:
        ...
        for code, positions in POSITIONS.items():
            ...
            for sort_order, (name_uz, name_ru) in enumerate(positions, start=1):
                existing = await session.scalar(
                    select(Position).where(
                        Position.category_id == category.id, Position.name_uz == name_uz
                    )
                )
                if existing:
                    continue
            ...
            logger.info(
                "%s toifasi uchun lavozimlar tekshirildi/qo'shildi (%d ta)",
                code,
                len(positions),
            )
        ...
        for key, texts in BOT_TEXTS.items():
            ...
            if existing:
                continue
            session.add(
                BotText(key=key, text_uz=texts["text_uz"], text_ru=texts["text_ru"])
            )
        ...
        if settings.bootstrap_super_admin_id:
            existing_admin = await session.scalar(
                select(Admin).where(
                    Admin.telegram_id == settings.bootstrap_super_admin_id
                )
            )
            if not existing_admin:
                existing_admin = Admin(
                    telegram_id=settings.bootstrap_super_admin_id,
                    full_name="Lazana Super Admin",
                    role="super_admin",
                )
                session.add(existing_admin)
                logger.info(
                    "Lazana super_admin qo'shildi: %s",
                    settings.bootstrap_super_admin_id,
                )
